...
Page({
            ...
            success: function (t) {
              ...
              for (var n in t.data) t.data[n].hours = t.data[n].time.slice(10, 19), t.data[n].time = (t.data[n].time,
                a = e = void 0, e = new Date(), (a = new Date(e)).setDate(e.getDate() + 7), a.getFullYear() + "-" + (a.getMonth() + 1) + "-" + a.getDate() + t.data[n].hours),
                 u <= t.data[n].time ? o.push(t.data[n]) : app.util.request({
  ...
  //获取动态接口
  getdtlist: function (e) {
    var that = this
    app.util.request({

      url: "entry/wxapp/dongtaiinfo",
      data: {
      ...
      success: function (res) {
        ...
        if (res.data.success) {
          ...
          if (res.data.data.type == 'add') {
            obj.user_is_dianzan = true
          } else {
            obj.user_is_dianzan = false
          }
          obj.dianzan_sum = res.data.data.dianzan_num
          that.setData({
            dongtailist: obj
          })
        }
    ...
    if (that.data.complete1 == null || that.data.complete1 == '') {
      ...
      app.util.request({
        ...
        data: {
          ...
          dongtaiid: that.data.dongtailist.id,  //动态id
          money: that.data.complete1, //打赏金额
          dongtai_userid: that.data.dongtailist.userid  //被打赏人
        },
        success: function (res) {
          //console.log(res)
          if (res.data.success) {
            ...
            for (var i = 0; i < obj.dashanglist.length; i++) {
              arr.dashanglist[i + 1] = obj.dashanglist[i]
            }
            obj.dashanglist = arr.dashanglist;
            that.setData({
              reward: !1,
              dongtailist: obj
            })
      ...
      app.util.request({
        ...
        success: function (res) {
          ...
          if (res.data.success) {
            ...
            for (var i = 0; i < obj.pinglunlist.length; i++) {
              arr.pinglunlist[i + 1] = obj.pinglunlist[i]
            }
            obj.pinglunlist = arr.pinglunlist;
            that.setData({
              comment: !1,
              dongtailist: obj
            })
    ...
    app.util.request({
      ...
      success: function (res) {
        ...
        if (res.data.info == '点赞成功') {
          ...
          obj.pinglunlist[t.currentTarget.dataset.plindex].heart = parseInt(obj.pinglunlist[t.currentTarget.dataset.plindex].heart) + 1
          obj.pinglunlist[t.currentTarget.dataset.plindex].user_is_dianzan = true
          ...
          obj.pinglunlist[t.currentTarget.dataset.plindex].heart = parseInt(obj.pinglunlist[t.currentTarget.dataset.plindex].heart) - 1
          obj.pinglunlist[t.currentTarget.dataset.plindex].user_is_dianzan = false
        }
        that.setData({
          dongtailist: obj
        })
  ...
  comdetail:function(e){
      wx.navigateTo({
        url: '../coments/coments?plid=' + e.currentTarget.dataset.plid + "&userid=" + wx.getStorageSync('users').id + "&dtid=" + this.data.dongtailist.id
      })
  }
});