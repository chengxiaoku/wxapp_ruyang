...
Page({
  ...
  uploadimg: function (e) {
    ...
    wx.uploadFile({
      ...
      formData: null,
      success: function (e) {
        "" != e.data ? (n++ , _imgArray.push(e.data), t.setData({
          imgArray1: _imgArray
        })) : wx.showToast({
          icon: "loading",
          title: "请重试"
  ...
  upvideo:function(e){
    var that = this
    wx.uploadFile({
      url: that.data.url + "app/index.php?i=" + wx.getStorageSync("uniacid") + "&c=entry&a=wxapp&do=UploadVideo&m=zh_tcwq",
      filePath: e,
      name: 'upfile',
      formData: null,
      success: function (res) {
        ...
        that.setData({
          video: e,
          videourl: JSON.parse(res.data).path
  ...
  //点击添加按钮
  selectwhat:function(e){
    var that = this;
    if (that.data.tapindex == -1 || (that.data.video == null && that.data.imgArray1=='')){
      wx.showActionSheet({
        itemList: ['照片', '视频'],
        success: function (res) {
          if (res.tapIndex==0) {
    ...
    } else if (that.data.tapindex == 0){
        that.imgArray1();
    } else if (that.data.tapindex == 1 && that.data.video==null) {
        that.addVideo()
    }
